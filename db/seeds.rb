...
Dir.foreach(preset_images_dir) do |directory_name|
  ...
  Dir.foreach(dir) do |file_name|
    ...
    gameObject = GameObject.create(name: 'test game object', text: 'game object explantion', game_id: game.id)
    gameObject.image.attach(io: File.open(dir + file_name), filename: file_name)

    PresetGameObject.create(groupe: directory_name, game_object_id: gameObject.id)
  end
end